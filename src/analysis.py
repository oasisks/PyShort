...
def generate_news_analysis_prompt(ticker: str, summary: str, prediction: int) -> str:
    """
    ...
    """
    news_analysis_prompt = (f"Instruction: Forecast the next day's stock return (price change) for {ticker}, given some news sources that came out "
...
tweets_example2 = ("Recent Tweets Summary: \n"
                   ...
                   "Stock Movement Prediction: Neutral \n \n"
                   "Stock Return: Neutral \n \n"
                   "Analysis: MSFT's stock is likely to remain stable, with no significant price change expected. Positive news about health-care AI tools offsets concerns about software bugs and performance, while the ML model's neutral indicator supports the forecast of minimal movement. Investors are likely to wait for further developments before making decisive moves."
                   )
...
tweets_example3 = ("Recent Tweets Summary: \n"
                   ...
                   "Keywords: \n AAPL, Apple, iOS 18, satellite phones, hiker, backpacker, stock, trading, options, calls, SPY, $572, profits, FOMC, crown, positive sentiment. \n \n"
                   "Stock Movement Prediction: Positive \n \n"
                   "Stock Return: Positive \n \n"
                   "Analysis: AAPL's stock is forecasted to increase by at least 3% due to widespread positive sentiment, strong product innovation (iOS 18’s features), and trader optimism reflected in calls and historical performance. The ML model's positive indicator supports this bullish outlook, with investors likely to drive demand for AAPL shares.")

def generate_tweets_analysis_prompt(ticker: str, summary: str, prediction: int) -> str:
    """
