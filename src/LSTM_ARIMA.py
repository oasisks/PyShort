...
def make_LSTM_pred(dataset, visualize = False):
    ...
    def make_and_fit_model(train_seq, train_label, test_seq, test_label):
        ...
        model = Sequential()
        model.add(LSTM(50, activation='relu', return_sequences=True, input_shape=(train_seq.shape[1], train_seq.shape[2])))
...
if __name__ == "__main__":
    ...
    ARIMA_results = make_ARIMA_pred(dataset, True)
    print(predicted_movement(*ARIMA_results))

    print("LSTM Prediction")
    LSTM_results = make_LSTM_pred(dataset, True)
